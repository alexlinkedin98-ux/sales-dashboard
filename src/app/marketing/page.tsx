        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          {viewMode === 'monthly' && monthlySummary && (
            <div className="space-y-6">
              ...
              {/* Per-Channel Cost Comparison - All Months */}
              {monthlyStats.length > 0 && (
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  <h2 className="text-xl font-semibold text-gray-900 mb-4">Channel Cost Comparison (All Time)</h2>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {(() => {
                          // Build per-channel aggregated stats
                          const channelCostData = channelNames.map((name) => {
                            const channel = channels.find((ch) => ch.name === name);
                            const channelId = channel?.id || '';
                            const totalLeads = channelTotals[name] || 0;
                            ...
                            return { name, totalLeads, totalSpend, totalClients, costPerLead, costPerAcquisition };
                          });

                          // Sort by cost per lead (lowest first) for comparison
                          const sortedByEfficiency = [...channelCostData].filter(c => c.costPerLead > 0).sort((a, b) => a.costPerLead - b.costPerLead);
                          const bestCostPerLead = sortedByEfficiency[0]?.costPerLead || 0;
                          ...
                          return channelCostData.map((ch) => (
                            <tr key={ch.name} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-center text-yellow-700 bg-yellow-50">
                                {ch.totalSpend > 0 ? `$${ch.totalSpend.toFixed(2)}` : '-'}
                              </td>
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-center text-purple-700 bg-purple-50">
                                {ch.totalClients > 0 ? ch.totalClients : '-'}
                              </td>
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-center bg-blue-50">
                                {ch.costPerLead > 0 ? (
                                  <span className={`font-medium ${ch.costPerLead === bestCostPerLead ? 'text-green-600' : 'text-blue-700'}`}>
                                    ${ch.costPerLead.toFixed(2)}
                                    {ch.costPerLead === bestCostPerLead && ' (Best)'}
                                  </span>
                                ) : (
                                  <span className="text-gray-400">-</span>
                                )}
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-center text-green-700 bg-green-50">
                                {ch.costPerAcquisition > 0 ? `$${ch.costPerAcquisition.toFixed(2)}` : '-'}
                              </td>
                            </tr>
                          ));
                        })()}
          ...
          {viewMode === 'quarterly' && (
            <div className="space-y-6">
              ...
              {/* Per-Channel Quarterly Cost Breakdown */}
              {monthlyStats.length > 0 && (
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  <h2 className="text-xl font-semibold text-gray-900 mb-4">Channel Cost Comparison (All Time)</h2>
                  <div className="overflow-x-auto">
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-center bg-green-50">
                                {ch.costPerAcquisition > 0 ? (
                                  <span className={`font-medium ${ch.costPerAcquisition === bestCAC ? 'text-green-600' : 'text-green-700'}`}>
                                    ${ch.costPerAcquisition.toFixed(2)}
                                    {ch.costPerAcquisition === bestCAC && ' (Best)'}
                                  </span>
                                ) : (
                                  <span className="text-gray-400">-</span>
                                )}
          ...
          {viewMode === 'annual' && (
            <div className="space-y-6">
              ...
              {/* Per-Channel Annual Cost Breakdown */}
              {monthlyStats.length > 0 && (
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  <h2 className="text-xl font-semibold text-gray-900 mb-4">Channel Cost Comparison (All Time)</h2>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {(() => {
                          ...
                          return channelCostData.map((ch) => {
                            const vsBestCPL = ch.costPerLead > 0 && bestCostPerLead > 0
                              ? ((ch.costPerLead - bestCostPerLead) / bestCostPerLead) * 100
                              : null;
